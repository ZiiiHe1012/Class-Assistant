...
> 基于 AI 的课堂学习助手 — 实时课件捕获解析 + 离线文档分析 + 笔记管理 + 翻译词典 + 智能对话
...
| **笔记系统** | 每张课件可独立记笔记；一键导入 AI 分析 / 深度思考；AI 生成结构化学习笔记（流式输出）；标签管理；全量导出 Markdown |
...
每张课件拥有独立笔记空间，数据以课件图片的 SHA1 哈希为键持久化到本地，重启服务或重新上传同一张图片后笔记内容不丢失。

点击解析面板顶部的 **「笔记」** 标签页即可切换到笔记视图。

| 功能 | 说明 |
|:---|:---|
| **手写笔记** | 支持 Markdown 语法，可随时切换「预览」模式渲染格式 |
| **导入分析** | 将当前课件的 AI 解析内容一键追加到笔记编辑器 |
| **导入深思** | 将「深度思考」结果追加到笔记编辑器 |
| **AI 生成笔记** | 点击「✦ AI 生成」，AI 自动生成包含核心概念、重要知识点、记忆技巧、可能考点的结构化笔记，流式实时输出 |
| **AI 笔记独立展示** | AI 生成笔记显示在紫色卡片区，与手写笔记分开；点击「追加到笔记」可合并 |
| **标签管理** | 输入标签后按 Enter 添加；内置快捷标签：重要、考点、复习、疑问；点击 × 删除 |
| **自动保存** | 停止输入 1.5 秒后自动保存，顶部显示「已保存」提示 |
| **笔记指示点** | 已记笔记的课件缩略图右下角显示绿点；已生成 AI 笔记的显示紫点 |
| **全量导出** | 点击「导出」按钮下载 `notes.md`，包含所有课件的手写笔记与 AI 笔记 |
...
<details>
<summary><b>Q: 重启服务后笔记还在吗？</b></summary>

在。笔记以课件图片的 SHA1 哈希为键，保存在 `data/notes/` 目录下的 JSON 文件中。只要重新上传或捕获同一张课件图片（哈希一致），笔记就会自动恢复显示。
</details>
...
│       └── notes-service.js       # 笔记 CRUD + AI 生成 + Markdown 导出
├── public/
...
└── data/
    ├── captures/              # 捕获的课件图片
    ├── notes/                 # 笔记文件（每张课件一个 JSON，以图片哈希命名）
    └── browser/               # 浏览器持久化数据
```
